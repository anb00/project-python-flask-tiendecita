...
def hello(category=None):
    ...
    print("This is category from Index",category)
    a = category
    ...
    if rows is not None:
     for x in rows:
        print(x)
    return render_template('test.html',  rows=rows,category=category,x=x,data=data,cats = cats)
...
def hola():
    ...
    return render_template('layout.html',posts=posts,cats=cats)
...
def index():
    ...
    # return render_template('index.html',cats=cats)
    # conn = get_db_connection()
    posts = conn.execute('SELECT * FROM item i inner join seller s on i.itemsellerid = s.sellerId').fetchall()
    conn.close()
